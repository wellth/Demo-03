...
#define DAEMON_DB_FILE			"MDB/bmi1.db"
//-----Used by UI---
...
//----------------------database loging--------------------------------------------------
						...
						if ((sql_errno = sqlite3_open(DAEMON_DB_FILE, &db)) != SQLITE_OK)
						{
	...
	//----------------------database loging--------------------------------------------------
					...
					//sprintf(mob_buf,"%ll",Current_MobileNo);
					printf("\nbp data---%s , %s , %ll ----\n", mob_buf, Uin.Name, Current_MobileNo);
...
void CheckUserInfo(void){
	...
	if(UserMode==1){
		...
		strcpy(mob_buf, Uin.Name);
		val = strtoll(Uin.Name,&endptr,10);
		...
		sprintf(query, "insert into login_t (mobilenumber, age, gender, log_time) values ('%s', '%s', '%s', '%s') ", mob_buf, Uin.Password, Uin.Gender, timeBuf);
...
void CompareUserInfo(void){
        ...
        if(fp1<0){
			...
			strcpy(login.LoginReply, "LOGIN_SUCCESS");
			strcpy(login.Name, Uin.Name);
			strcpy(login.Age, Uin.Password);
			strcpy(login.Gender, Uin.Gender);			
