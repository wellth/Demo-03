...
#define ENABLE_BLOCKREAD_TIMEOUT

#define BLOCKREAD_TIMEOUT_SEC 	1
...
//	options.c_cflag |= (CRTSCTS | CREAD | CWRITE);
...
int ArgOpenSerialPort(char *SerialPortName, int baudRate){
	...
	if(fd < 0) {
 	...
 	{
		g_print("Able to open the SerialPort %s \n ",SerialPortName);

    	if(nu_SerialComPort_Init(fd, baudRate) < 0 ){
    		g_print("baud not set");
		    return -1;
    	}
	}
	return fd;
